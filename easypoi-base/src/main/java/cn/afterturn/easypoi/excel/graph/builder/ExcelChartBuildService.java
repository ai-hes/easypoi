...
public class ExcelChartBuildService {
    ...
    private static void buildExcelChart(Drawing drawing, ClientAnchor anchor, Sheet dataSourceSheet, ExcelGraph graph) {
        ...
        if (drawing instanceof XSSFDrawing) {
            chart = ((XSSFDrawing)drawing).createChart(anchor);
            ...
            if (categoryElement != null && categoryElement.getElementType().equals(ExcelGraphElementType.STRING_TYPE)) {
                categoryChart = DataSources.fromStringCellRange(dataSourceSheet,
                    new CellRangeAddress(categoryElement.getStartRowNum(), categoryElement.getEndRowNum(),
                        categoryElement.getStartColNum(), categoryElement.getEndColNum()));
            } else {
            ...
            if (valueList != null && valueList.size() > 0) {
                for (ExcelGraphElement ele : valueList) {
                    ChartDataSource<Number> source = DataSources.fromNumericCellRange(dataSourceSheet,
                        new CellRangeAddress(ele.getStartRowNum(), ele.getEndRowNum(), ele.getStartColNum(),
                            ele.getEndColNum()));
    ...
    private static void buildLineChartData(LineChartData data, ChartDataSource categoryChart,
        List<ChartDataSource<Number>> chartValueList, List<String> title) {
